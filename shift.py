...
for i in number:
    path1=os.path.join(path,'merge_'+str(i)+'_2.csv')
    ...
    days = pd.read_csv(path1, usecols=[0,15])
    ...
    df_train5=pd.merge(df_train4,days,on='date')
    df_train5=df_train5.dropna()
    df_train5.rename(columns={'T20_mean': 'T20_mean_z', 'ST_mean': 'ST_mean_z', 'T60_mean':'T60_mean_z','H_mean':'H_mean_z', 'T20_min':'T20_min_z', 'ST_min':'ST_min_z', 'T60_min':'T60_min_z', 'H_min':'H_min_z', 'T20_max':'T20_max_z','ST_max':'ST_max_z', 'T60_max':'T60_max_z', 'H_max':'H_max_z','housa':'housa_z'},inplace=True)
    df_train5.drop_duplicates(subset='date')
    df_train5=df_train5.set_index('date')
    df_train5.to_csv(os.path.join(save_path,'merge_'+str(i)+'_train_2.csv'),encoding='utf_8',index=True)
    ...
    df_test5=pd.merge(df_test4,days,on='date')
    df_test5=df_test5.dropna()
    df_test5.rename(columns={'T20_mean': 'T20_mean_z', 'ST_mean': 'ST_mean_z', 'T60_mean':'T60_mean_z','H_mean':'H_mean_z', 'T20_min':'T20_min_z', 'ST_min':'ST_min_z', 'T60_min':'T60_min_z', 'H_min':'H_min_z', 'T20_max':'T20_max_z','ST_max':'ST_max_z', 'T60_max':'T60_max_z', 'H_max':'H_max_z','housa':'housa_z'},inplace=True)
    df_test5.drop_duplicates(subset='date')
    df_test5=df_test5.set_index('date')
    df_test5.to_csv(os.path.join(save_path,'merge_'+str(i)+'_test_2.csv'),encoding='utf_8',index=True)
#%% データセットall作成
csv_cont_all_train= []
csv_cont_all_test= []
number=['number1','number2','number3']
...
for i in dataset:
    df=pd.read_csv(os.path.join(path,'excel','merge_'+str(i)+'_test_2.csv'))
